...
try{
    const response =  await axios.get("https://eduweb-project.onrender.com/courses/get-courses",
          {
 ...
 setcourses(response.data.courses||[])
  if (response.status === 201 || response.status === 200) {
          console.log("successful retreiving all courses !");
                ...
                console.log(error);
                 setError(`Error inrolling: ${ error.response.data.msg|| error.message}`)
              }
            }
             fetchCourses();
}, []);
const handleEnroll = async(course) => {
    try{
      sessionStorage.setItem("enrolledcourseid",course._id)
        ...
        const response = await axios.post(`https://eduweb-project.onrender.com/student/inroll-student/${course._id}`,{},{
            headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` },
          },)
          console.log("heremmm");
